"""
main_facebook.py — Facebook Reels Automation Orchestrator

Runs the full pipeline VIDEOS_PER_RUN times:
  Gemini script → AI images → edge-tts voiceover → FFmpeg video → Facebook Reel upload

Usage:
  python main_facebook.py              # full pipeline (all 6 steps)
  python main_facebook.py --steps 5   # stop after video assembly (skip upload)
  python main_facebook.py --steps 3   # stop after voiceover (skip video + upload)
...
Steps:
  ...
  4 = Background music
  5 = Video assembly
  6 = Facebook Reel upload (default)

Required environment variables / GitHub Secrets:
  GEMINI_API_KEY            — Gemini 2.5 Flash (script + caption generation)
  UNSPLASH_ACCESS_KEY       — Stock images
  PEXELS_API_KEY            — Stock images (fallback)
  FACEBOOK_PAGE_ID          — Numeric ID of your Facebook Page
  FACEBOOK_PAGE_ACCESS_TOKEN — Long-lived Page Access Token (see src/facebook_uploader.py)
"""
...
from src.script_generator import generate_facebook_video_content
...
from src.facebook_uploader import upload_video_facebook
...
def run_pipeline(video_index: int, stop_after: int = 6) -> dict:
    ...
    try:
        # ── 1. Script ─────────────────────────────────────────────────────────
        print("\n[1/6] Generating script for Facebook (Gemini 2.5 Flash)…")
        content = generate_facebook_video_content()
        ...
        print("\n[6/6] Uploading to Facebook as Reel…")
        video_id = upload_video_facebook(
        ...
        # Clean up temp dir only after a full successful run
        if stop_after >= 6:
...
def main() -> None:
    ...
    print(f"\nFacebook Reels Automation — {start.strftime('%Y-%m-%d %H:%M UTC')}")
    ...
    print(f"\n{'=' * 56}")
    if args.steps == 6:
        print(f"DONE — {len(results)}/{VIDEOS_PER_RUN} reels uploaded in ~{elapsed} min")
        for r in results:
            if "url" in r:
                print(f"  + {r['title'][:60]}")
                print(f"    {r['url']}")
    ...
    for f in failures:
        print(f"  x Video {f['index']} — {f['error'][:80]}")
    print("=" * 56)
